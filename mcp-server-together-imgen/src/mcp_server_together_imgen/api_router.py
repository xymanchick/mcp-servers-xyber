...
async def generate_image(
    ...
    try:
        ...
        prompt_looks_like_model = (
            ...
            or any(
                model in request.prompt.lower()
                for model in ["flux.1", "flux.2", "flux-1", "flux-2"]
            )
        )
        ...
        if request.model:
            ...
            try:
                ...
                if (
                    request.steps is not None
                    and not model_schema.capabilities.supports_steps
                ):
                    logger.warning(
                        f"Parameter 'steps' is not supported for {request.model}. It will be automatically removed."
                    )
                if (
                    request.guidance_scale is not None
                    and not model_schema.capabilities.supports_guidance_scale
                    and not model_schema.capabilities.supports_guidance_param
                ):
                    logger.warning(
                        f"Parameter 'guidance_scale' is not supported for {request.model}. It will be automatically removed."
                    )
                if (
                    request.negative_prompt is not None
                    and not model_schema.capabilities.supports_negative_prompt
                ):
                    logger.warning(
                        f"Parameter 'negative_prompt' is not supported for {request.model}. It will be automatically removed."
                    )
                if (
                    request.lora_scale
                    and request.lora_scale > 0
                    and not model_schema.capabilities.supports_lora
                ):
                    logger.warning(
                        f"Parameter 'lora_scale' is not supported for {request.model}. It will be automatically removed."
                    )
        ...
        if (
            ...
            or "remove this parameter" in error_msg.lower()
        ):
