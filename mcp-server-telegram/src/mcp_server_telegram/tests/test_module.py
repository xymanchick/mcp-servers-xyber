...
class TestTelegramService:
    ...
    async def test_send_message_missing_token(self, mock_config_empty_token):
        ...
        with pytest.raises(
            TelegramConfigError, match="Telegram token or channel is missing"
        ):
    ...
    async def test_send_message_success(
        self, mock_post, mock_config, mock_success_response
    ):
        # Setup
        service = _TelegramService(mock_config)
    ...
    @pytest.mark.asyncio
    @patch("mcp_server_telegram.telegram.module.requests.post")
    async def test_send_message_truncate_long_text(
        self, mock_post, mock_config, mock_success_response
    ):
        service = _TelegramService(mock_config)
        mock_post.return_value = mock_success_response
    ...
    @pytest.mark.asyncio
    @patch("mcp_server_telegram.telegram.module.requests.post")
    async def test_send_message_retry_on_parse_error(
        self,
        mock_post,
        mock_config,
        mock_400_parse_error_response,
    ...
    @pytest.mark.asyncio
    @patch("mcp_server_telegram.telegram.module.requests.post")
    async def test_send_message_400_error_not_parse_related(
        self, mock_post, mock_config, mock_400_other_error_response
    ):
        service = _TelegramService(mock_config)
...
class TestGetTelegramService:
    ...
    @patch("mcp_server_telegram.telegram.module.TelegramConfig")
    def test_get_telegram_service_different_params(
        self, mock_config_class, create_mock_config
    ):
        get_telegram_service.cache_clear()
    ...
    def test_get_telegram_service_config_error(self, mock_config_class):
        ...
        with pytest.raises(
            TelegramConfigError, match="Configuration error for Telegram service"
        ):
...
class TestTelegramServiceAdditional:
    @pytest.mark.asyncio
    @patch("mcp_server_telegram.telegram.module.requests.post")
    async def test_send_message_http_error_no_json(
        self, mock_post, mock_config, mock_500_error_response
    ):
        service = _TelegramService(mock_config)

        mock_500_error_response.raise_for_status.side_effect = HTTPError(
            response=mock_500_error_response
        )
    ...
    @pytest.mark.asyncio
    @patch("mcp_server_telegram.telegram.module.requests.post")
    async def test_send_message_400_parse_error_variations(
        self, mock_post, mock_config, mock_success_response
    ):
        service = _TelegramService(mock_config)
        ...
        first_response.json.return_value = {
            "description": "Bad Request: parse error at position 5"
        }
