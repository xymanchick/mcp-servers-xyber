...
class TokenSearchAPI:
    ...
    async def search_token(self, query: str, chain_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
            ...
            for key in keys_to_check:
                if key in known_tokens:
                    ...
                    try:
                        ...
                        async with httpx.AsyncClient(timeout=10.0) as client:
                            ...
                            if pairs:
                                ...
                                if token_info.get("symbol", "").upper() == query_upper:
                                    logger.info(f"Using known token address for {query_upper}")
                                    return {
                                        "address": known_addr,
                        ...
                        logger.debug(f"Could not fetch info for known address {known_addr}: {e}")
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                try:
                    ...
                    if chain_id:
                        ...
                        filtered_pairs = [
                            p for p in pairs 
                            if aliases.get(p.get("chainId", "").lower(), p.get("chainId", "").lower()) == target_canonical
                        ]
                        ...
                        if exact_matches:
                            ...
                            if pair.get("baseToken", {}).get("symbol", "").upper() != query_upper:
                                logger.warning(f"Symbol mismatch: expected {query_upper}, got {pair.get('baseToken', {}).get('symbol')}")
                                continue  # Try next query variation
                        else:
                            # No exact match - reject if we're looking for a specific symbol
                            logger.warning(f"No exact symbol match for {query_upper} on {target_canonical}")
                            continue  # Try next query variation
                    else:
                    ...
                    if chain_id and result_symbol != query_upper:
                        logger.warning(f"Rejecting result: symbol {result_symbol} doesn't match query {query_upper}")
                        continue  # Try next query variation
                    ...
                    logger.info(f"Found token: {result['name']} ({result['symbol']}) at {result['address']} on {result['chainId']} using query '{q}'")
                    return result
        ...
        logger.warning(f"No tokens found for {query} on chain {chain_id} after trying all variations.")
        return None
