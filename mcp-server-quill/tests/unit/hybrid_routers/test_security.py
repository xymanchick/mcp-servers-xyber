...
async def test_get_evm_token_info_success(client: AsyncClient) -> None:
    with (
        patch(
            "mcp_server_quill.quill.search.TokenSearchAPI.search_token",
            new_callable=AsyncMock,
        ) as mock_search,
        patch(
            "mcp_server_quill.quill.client.QuillAPI.get_token_info",
            new_callable=AsyncMock,
        ) as mock_quill,
    ):
...
async def test_get_solana_token_info_success(client: AsyncClient) -> None:
    with (
        ...
        patch(
            "mcp_server_quill.quill.client.QuillAPI.get_solana_token_info",
            new_callable=AsyncMock,
        ) as mock_quill,
    ):
