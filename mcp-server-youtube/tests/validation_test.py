...
def test_valid_request_minimal():
    ...
    response = client.post(
        ...
        json={"request": {"query": "python programming", "max_results": 3}},
    )
    assert (
...
def test_valid_request_with_language():
    ...
    for video in data["videos"]:
        assert (
            ...
            or video["transcript"] != "[Transcripts are disabled]"
        )
...
def test_valid_request_with_default_max_results():
    ...
    response = client.post(
        "/youtube_search_and_transcript",
        json={"request": {"query": "python programming", "max_results": 5}},
    )
    assert (
        response.status_code == 200
    ), f"Response status: {response.status_code}, Content: {response.json()}"
...
def test_empty_query():
    ...
    response = client.post(
        "/youtube_search_and_transcript",
        json={"request": {"query": "", "max_results": 5}},
    )
...
def test_invalid_max_results():
    ...
    response = client.post(
        "/youtube_search_and_transcript",
        json={"request": {"query": "python programming", "max_results": 0}},
    )
    assert response.status_code == 400
    data = response.json()
    assert "error" in data
...
def test_invalid_max_results_too_high():
    ...
    response = client.post(
        "/youtube_search_and_transcript",
        json={"request": {"query": "python programming", "max_results": 25}},
    )
    assert response.status_code == 400
    data = response.json()
    assert "error" in data
...
def test_too_long_query():
    ...
    response = client.post(
        "/youtube_search_and_transcript",
        json={"request": {"query": long_query, "max_results": 5}},
    )
    assert response.status_code == 400
    data = response.json()
    assert "error" in data
...
def test_negative_max_results():
    ...
    response = client.post(
        "/youtube_search_and_transcript",
        json={"request": {"query": "python programming", "max_results": -1}},
    )
    assert response.status_code == 400
    data = response.json()
    assert "error" in data
