...
class TestRetryTranscriptApiDecorator:
    ...
    def test_retry_on_transcript_error_then_success(self, youtube_searcher, caplog):
        ...
        with patch(
            "mcp_server_youtube.youtube.module.TranscriptFetcher"
            ...
            with caplog.at_level(logging.WARNING):
                result = youtube_searcher._get_transcript_by_id(
                    TEST_VIDEO_IDS["video1"], "en"
                )
    ...
    def test_retry_logging_details(self, youtube_searcher, caplog):
        ...
        assert (
            "1.0 seconds" in retry_logs[1].message
            or "1 second" in retry_logs[1].message
        )
...
class TestIntegratedRetryBehavior:
    ...
    def test_search_with_transcript_retries(self, youtube_searcher, caplog):
        ...
        # Verify log content
        search_retry_logs = [
            log for log in retry_logs if "search_videos" in log.message
        ]
        transcript_retry_logs = [
            log for log in retry_logs if "_get_transcript_by_id" in log.message
        ]
