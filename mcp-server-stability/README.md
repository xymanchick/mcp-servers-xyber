...
| `POST /hybrid/generate-image`    | **Paid**   | Generates an image from a text prompt using Stability AI      |
...
Currently, there are no MCP-only endpoints in this server. All tools are accessible via both REST and MCP (hybrid).
