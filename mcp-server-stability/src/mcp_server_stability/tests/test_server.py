...
class TestAppLifespan:
    ...
    async def test_app_lifespan_success(self, mock_stability_service):
        ...
        with patch('mcp_server_stability.server.get_stability_service', 
    ...
    @pytest.mark.asyncio
    async def test_app_lifespan_cleanup_called_on_exception(self, mock_stability_service):
        mock_server = Mock(spec=FastMCP)
...
class TestGenerateImage:
    @pytest.mark.asyncio
    async def test_generate_image_success(self, mock_context, mock_stability_service, 
                                        sample_image_data, sample_base64_image):
        mock_context.request_context.lifespan_context["stability_service"] = mock_stability_service
        ...
        mock_stability_service.send_generation_request.assert_called_once_with(expected_params)

    @pytest.mark.asyncio
    async def test_generate_image_with_defaults(self, mock_context, mock_stability_service, 
                                              sample_image_data, sample_base64_image):
        mock_context.request_context.lifespan_context["stability_service"] = mock_stability_service
    ...
    @pytest.mark.asyncio
    async def test_generate_image_without_style_preset(self, mock_context, mock_stability_service,
                                                     sample_image_data):
        mock_context.request_context.lifespan_context["stability_service"] = mock_stability_service
    ...
    @pytest.mark.asyncio
    async def test_generate_image_content_filtered(self, mock_context, mock_stability_service):
        mock_context.request_context.lifespan_context["stability_service"] = mock_stability_service
    ...
    @pytest.mark.asyncio
    async def test_generate_image_server_connection_error(self, mock_context, mock_stability_service):
        mock_context.request_context.lifespan_context["stability_service"] = mock_stability_service
        
        error_msg = "Connection timeout"
        mock_stability_service.send_generation_request.side_effect = StableDiffusionServerConnectionError(error_msg)
    ...
    @pytest.mark.asyncio
    async def test_generate_image_client_error(self, mock_context, mock_stability_service):
        mock_context.request_context.lifespan_context["stability_service"] = mock_stability_service
        
        error_msg = "Invalid API key"
        mock_stability_service.send_generation_request.side_effect = StableDiffusionClientError(error_msg)
    ...
    @pytest.mark.asyncio
    async def test_generate_image_unexpected_error(self, mock_context, mock_stability_service):
        mock_context.request_context.lifespan_context["stability_service"] = mock_stability_service
        
        error_msg = "Unexpected network error"
        mock_stability_service.send_generation_request.side_effect = RuntimeError(error_msg)
    ...
    @pytest.mark.asyncio
    async def test_generate_image_different_aspect_ratios(self, mock_context, mock_stability_service,
                                                         sample_image_data, sample_base64_image):
        mock_context.request_context.lifespan_context["stability_service"] = mock_stability_service
    ...
    @pytest.mark.asyncio
    async def test_generate_image_different_style_presets(self, mock_context, mock_stability_service,
                                                         sample_image_data, sample_base64_image):
        mock_context.request_context.lifespan_context["stability_service"] = mock_stability_service
    ...
    @pytest.mark.asyncio
    async def test_generate_image_with_zero_seed(self, mock_context, mock_stability_service,
                                                sample_image_data, sample_base64_image):
        mock_context.request_context.lifespan_context["stability_service"] = mock_stability_service
