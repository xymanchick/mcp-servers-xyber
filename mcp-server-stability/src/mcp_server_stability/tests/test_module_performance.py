...
class TestStabilityServicePerformance:
    ...
    @pytest.mark.asyncio
    async def test_memory_usage_with_multiple_requests(
        self, mock_stability_config, mock_successful_response
    ):
        service = self.StabilityService(mock_stability_config)
    ...
    @pytest.mark.asyncio
    async def test_client_reuse_efficiency(
        self, mock_stability_config, mock_successful_response
    ):
        service = self.StabilityService(mock_stability_config)
...
class TestStabilityServiceLoadTesting:
    ...
    async def test_high_concurrency_load(
        ...
        params_list = [{"prompt": f"Load test {i}"} for i in range(high_load_count)]
    ...
    @pytest.mark.asyncio
    async def test_long_running_service(
        self, mock_stability_config, mock_successful_response
    ):
        ...
        with patch("httpx.AsyncClient", return_value=mock_client):
            ...
            for batch in range(5):  # 5 батчей
                ...
                assert all(
                    response == mock_successful_response for response in responses
                )
