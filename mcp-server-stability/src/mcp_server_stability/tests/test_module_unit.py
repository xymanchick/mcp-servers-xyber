...
class TestStabilityServiceUnit:
    ...
    def test_init(self, mock_stability_config):
        ...
        assert service.host == "https://api.stability.ai/v2beta/stable-image/generate/core"
    ...
    @pytest.mark.asyncio
    async def test_send_generation_request_success(self, mock_stability_config, sample_generation_params, mock_successful_response):
        service = self.StabilityService(mock_stability_config)
    ...
    @pytest.mark.asyncio
    async def test_send_generation_request_api_error(self, mock_stability_config, mock_error_response):
        service = self.StabilityService(mock_stability_config)
    ...
    @pytest.mark.asyncio
    async def test_multiple_requests_same_client(self, mock_stability_config, mock_successful_response):
        service = self.StabilityService(mock_stability_config)
...
class TestStabilityServiceEdgeCases:
    ...
    @pytest.mark.asyncio 
    async def test_concurrent_requests(self, mock_stability_config, mock_successful_response):
        service = self.StabilityService(mock_stability_config)
        
        params_list = [
            {"prompt": f"Image {i}"} for i in range(3)
        ]
    ...
    @pytest.mark.asyncio
    async def test_empty_params_handling(self, mock_stability_config, mock_successful_response):
        service = self.StabilityService(mock_stability_config)
    ...
    @pytest.mark.asyncio
    async def test_large_params_handling(self, mock_stability_config, mock_successful_response):
        service = self.StabilityService(mock_stability_config)
