...
async def payment_app(
    ...
    monkeypatch.setattr(
        "mcp_server_arxiv.middlewares.x402_wrapper.FacilitatorClient",
...
async def test_valid_payment_header_allows_request_and_sets_response_header(
    ...
    resp_paid = await client.post("/hybrid/search", headers=headers)
...
async def test_payment_header_with_wrong_network_returns_no_matching(
    ...
    resp = await client.post("/hybrid/search", headers={"X-PAYMENT": tampered_header})
