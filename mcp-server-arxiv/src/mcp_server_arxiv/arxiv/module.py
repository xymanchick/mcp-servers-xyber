...
@retry(
    ...
    stop=stop_after_attempt(os.getenv("ARXIV_MAX_RETRIES", 5)),
)
async def _async_download_and_extract_pdf_text(
    ...
    try:
        ...
        await loop.run_in_executor(
            None, lambda: paper.download_pdf(dirpath=temp_dir, filename=safe_filename)
        )
        ...
        if not full_text_content:
            ...
            full_text_content = (
                full_text_content[:max_text_length]
                + f"... (truncated to {max_text_length} chars)"
            )
        logger.info(
            f"  Successfully extracted text for {arxiv_id} (length: {len(full_text_content)} chars)."
        )
    ...
    #     logger.error(f"  Error for {arxiv_id}: {processing_error_message}")
        ...
        logger.error(f"  Error for {arxiv_id}: {processing_error_message}")
        raise ArxivApiError(
            f"HTTP error while downloading PDF for {arxiv_id}: {e}"
        ) from e
    except urllib.error.URLError as e:
        if isinstance(e.reason, socket.timeout):
            processing_error_message = "[Download timed out]"
        elif isinstance(e.reason, socket.gaierror):
            processing_error_message = "[Network error: DNS resolution failed]"
        elif isinstance(e.reason, ConnectionRefusedError):
            processing_error_message = "[Network error: Connection refused]"
        else:
            processing_error_message = f"[Network error: {e.reason}]"

        logger.error(f"  Error for {arxiv_id}: {processing_error_message}")
        raise ArxivApiError(
            f"Network error while downloading PDF for {arxiv_id}: {processing_error_message}"
        ) from e
    except (
        fitz.FileDataError,
        fitz.EmptyFileError,
        ...
        logger.error(
            f"  Error for {arxiv_id}: {processing_error_message}", exc_info=True
        )
    finally:
...
class _ArxivService:
    ...
    def __init__(self, config: ArxivConfig):
        ...
        if arxiv is None or fitz is None:
            logger.error(
                "arxiv or PyMuPDF (fitz) package not installed. Arxiv service unavailable."
            )
            raise ArxivConfigError(
                "Required package 'arxiv' or 'PyMuPDF' not installed."
            )
    ...
    async def search(
        ...
        effective_max_text_length = (
            ...
            else self.config.default_max_text_length
        )
        ...
        try:
            ...
            try:
                results_iterator = await loop.run_in_executor(
                    None, self.client.results, search_params
                )
                # Convert iterator to list within the executor to avoid StopIteration issues in async context
                papers_metadata = await loop.run_in_executor(
                    None, list, results_iterator
                )
            except (
                Exception
            ) as e:  # Catch broad exceptions from arxiv library during initial fetch
                logger.error(
                    f"Error fetching initial results from ArXiv for query '{query}': {e}",
                    exc_info=True,
                )
                raise ArxivApiError(
                    f"Failed to retrieve search results from ArXiv: {e}"
                ) from e
            ...
            with tempfile.TemporaryDirectory(prefix="arxiv_pdf_") as temp_dir:
                ...
                for (
                    paper_obj
                ) in papers_metadata:  # papers_metadata is now a list of arxiv.Result
                    if not isinstance(paper_obj, arxiv.Result):
                        logger.warning(
                            f"Skipping non-Result item from ArXiv: {type(paper_obj)}"
                        )
                        continue
                    ...
                    task = asyncio.create_task(
                        _async_download_and_extract_pdf_text(
                            paper_initial_details, temp_dir, effective_max_text_length
                        ),
                ...
                for i, res_or_exc in enumerate(processed_results):
                    if isinstance(res_or_exc, ArxivSearchResult):
                        ...
                        original_paper_obj = papers_metadata[
                            i
                        ]  # Relies on order preservation
                        logger.error(
                            f"Task for paper {original_paper_obj.get_short_id()} failed: {res_or_exc}",
                            exc_info=res_or_exc,
                        )
                        ...
                        logger.warning(
                            f"Unexpected item in gathered results: {type(res_or_exc)}"
                        )
...
#            raise
            ...
            logger.error(
                f"Error during ArXiv search operation for query '{query}': {e}",
                exc_info=True,
            )
            # This is a catch-all for unexpected issues in the orchestration
            raise ArxivServiceError(
                f"An unexpected error occurred during the ArXiv search operation: {e}"
            ) from e
...
def get_arxiv_service() -> _ArxivService:
    ...
    try:
        ...
        ArxivConfigError
    ) as e:  # Handles errors from ArxivConfig() or _ArxivService() init
        ...
        logger.error(
            f"FATAL: Unexpected error initializing _ArxivService: {e}", exc_info=True
        )
        raise ArxivConfigError(
            f"Unexpected error during _ArxivService initialization: {e}"
        ) from e
