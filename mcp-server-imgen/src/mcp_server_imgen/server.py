...
async def app_lifespan(server: FastMCP) -> AsyncIterator[dict[str, Any]]:
    ...
    try:
        ...
        image_generator: _ImageGenerationService = get_image_generation_service(google_service)
...
async def generate_image(
    ...
    seed: Annotated[int | None, Field(None, description="Seed for reproducible generation")] = None,
    guidance_scale: Annotated[float, Field(7.5, description="Prompt guidance scale (1.0-20.0)", ge=1.0, le=20.0)] = 7.5,
    num_inference_steps: Annotated[int, Field(50, description="Number of denoising steps (10-100)", ge=10, le=100)] = 50,
    validation_retries: Annotated[int, Field(0, description="Number of times to retry validation if it fails", ge=0, le=50)] = 0
) -> str:
    ...
    for attempt in range(validation_retries + 1):
        try:
            ...
            raise ToolError(f"Image generation service error: {service_err}") from service_err
