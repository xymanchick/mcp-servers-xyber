"""
Configuration module for x402 payment protocol.

Loads x402-specific settings from environment variables and YAML pricing files.
"""
...
# File location: <repo>/src/mcp_twitter/x402_config.py -> repo root is 2 levels up.
_project_root = Path(__file__).resolve().parents[2]
_env_file = _project_root / ".env"
load_dotenv(dotenv_path=_env_file)


# Mapping of chain IDs to network names (for logging and validation)
CHAIN_ID_TO_NETWORK = {
    1: "ethereum-mainnet",
    8453: "base-mainnet",
    84532: "base-sepolia",
    ...
    def validate_pricing_mode(self) -> None:
        ...
        if self.pricing_mode == "on" and not has_pricing:
            raise ValueError(
                ...
                "Either set MCP_TWITTER_X402_PRICING_MODE=off or provide a valid "
        ...
        if self.pricing_mode == "off" and has_pricing:
            logger.warning(
                ...
                "Set MCP_TWITTER_X402_PRICING_MODE=on to enable payments."
            )
