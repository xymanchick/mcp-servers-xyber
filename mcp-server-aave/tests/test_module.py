...
class TestAaveClient:
    ...
    def test_cache_key_generation(self, mock_aave_config):
        ...
        client = AaveClient(mock_aave_config)
        key = client._get_cache_key("get_markets_data", chain_ids=[1, 137])
        
        assert key == "get_markets_data:chain_ids=[1, 137]"
        
        # Test with multiple parameters
        key = client._get_cache_key("get_asset_risk_data", asset_address="0x123", network="polygon")
        assert key == "get_asset_risk_data:asset_address=0x123:network=polygon"
    ...
    @pytest.mark.asyncio
    async def test_get_markets_data_success(self, mock_aave_config, sample_aave_api_response):
        """Test successful market data retrieval."""
        client = AaveClient(mock_aave_config)
        
        with aioresponses() as mocked:
            # Mock the API response
            mocked.post(mock_aave_config.api_base_url, status=200, payload=sample_aave_api_response)
            
            result = await client.get_markets_data(chain_ids=[1])
            
            assert isinstance(result[0], PoolData)
            assert result[0].chain.name == "Ethereum"
            assert len(result[0].reserves) == 1
            assert result[0].total_market_size == 2500000000
            
        # Clean up
        await client.close()
    
    @pytest.mark.asyncio
    async def test_get_markets_data_from_cache(self, mock_aave_config, sample_pool_data):
        """Test retrieving market data from cache."""
        client = AaveClient(mock_aave_config)
        
        # Add item to cache
        cache_key = client._get_cache_key("get_markets_data", chain_ids=[1])
        client._cache[cache_key] = (time.time(), [sample_pool_data])
        ...
        with aioresponses() as mocked:
            ...
            assert result[0] == sample_pool_data
    ...
    @pytest.mark.asyncio
    async def test_get_markets_data_api_error(self, mock_aave_config):
        """Test handling of API errors."""
        client = AaveClient(mock_aave_config)
        
        with aioresponses() as mocked:
            # Mock API error response
            mocked.post(mock_aave_config.api_base_url, status=500, payload={"error": "Internal server error"})
            
            with pytest.raises(AaveApiError, match="AAVE API HTTP error: 500"):
                await client.get_markets_data(chain_ids=[1])
        
        # Clean up
        await client.close()
    
    @pytest.mark.asyncio
    async def test_get_markets_data_request_exception(self, mock_aave_config):
        """Test handling of request exceptions."""
        client = AaveClient(mock_aave_config)
        
        with aioresponses() as mocked:
            # Mock connection error
            mocked.post(mock_aave_config.api_base_url, exception=aiohttp.ClientConnectionError("Connection failed"))
    ...
    @pytest.mark.asyncio
    async def test_get_asset_risk_data_success(self, mock_aave_config, sample_risk_data, sample_pool_data):
        """Test successful risk data retrieval."""
        client = AaveClient(mock_aave_config)
        
        with patch.object(client, 'get_markets_data', new_callable=AsyncMock) as mock_get_markets:
            mock_get_markets.return_value = [sample_pool_data]
