...
class HealthChecker:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        for op_metrics in self.metrics.operations.values():
            if (op_metrics.last_called and 
                now - op_metrics.last_called < timedelta(minutes=5)):
                recent_activity = True
                break
        
        health_status["checks"]["recent_activity"] = {
            "status": "pass" if recent_activity else "warn",
            "message": "Recent activity detected" if recent_activity else "No recent activity"
        }
