...
async def follow_user(
    ...
    try:
        operation_logger.debug(f"Calling Twitter API to follow user {follow_request.user_id}")
        ...
        raise ToolError(f"Error following user {follow_request.user_id}: {str(e)}") from e
        ...
        raise ToolError(f"An unexpected error occurred while following user {follow_request.user_id}: {str(e)}") from e
