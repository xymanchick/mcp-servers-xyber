    ...
    try:
        ...
        for uid in get_request.user_ids:
            user_logger = get_logger(
                f"{__name__}.get_user_tweets", operation="get_user_tweets", user_id=uid
            )
        ...
        operation_logger.info(
            ...
            extra={
                "successful_users": len(
                    [
                        uid
                        for uid, tweets in tweets_dict.items()
                        if not any("Error:" in tweet for tweet in tweets)
                    ]
                ),
                "failed_users": len(
                    [
                        uid
                        for uid, tweets in tweets_dict.items()
                        if any("Error:" in tweet for tweet in tweets)
                    ]
                ),
                "total_tweets": sum(
                    len(tweets)
                    for tweets in tweets_dict.values()
                    if not any("Error:" in tweet for tweet in tweets)
                ),
            },
        )
