...
def create_final_retry_exception(retry_state, base_exception_class=TwitterAPIError):
    ...
    context = {
        ...
        "final_error_type": type(original_exception).__name__
    ...
    return base_exception_class(
        message=error_message, context=context, original_exception=original_exception
    )
...
def on_final_retry_failure(retry_state):
    ...
    logger.error(
        ...
        extra={
            ...
            "final_exception_type": type(original_exception).__name__
            if original_exception
            else "Unknown",
            "final_error_message": str(original_exception)
            if original_exception
            else "Unknown error",
        },
    )
...
def map_tweepy_error(
    ...
    if status_code == 401:
        return TwitterAuthenticationError(
            message=message, context=context, original_exception=exception
        )
