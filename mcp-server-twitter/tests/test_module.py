...
@pytest.mark.asyncio
async def test_create_tweet_retries_on_aiohttp_client_error(mock_config, mocker, caplog):
    """
...
async def test_create_tweet_fails_after_max_retries(mock_config, mocker):
    ...
    mock_async_client.create_tweet.side_effect = aiohttp.ClientError("Persistent failure")
