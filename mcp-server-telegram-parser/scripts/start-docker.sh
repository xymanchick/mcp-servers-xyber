#!/bin/bash
# Start Telegram Parser MCP Server in Docker (standalone)
...
REBUILD=false
PORT=8012
CONTAINER_NAME="mcp-server-telegram-parser"
IMAGE_NAME="mcp-server-telegram-parser"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --port PORT            Specify port (default: 8012)"
            echo "  --container-name NAME  Specify container name (default: mcp-server-telegram-parser)"
            echo "  --image-name NAME      Specify image name (default: mcp-server-telegram-parser)"
...
echo "Starting Telegram Parser MCP Server in Docker..."
...
docker run -d \
  ...
  --network host \
  --env-file .env \
  -e "MCP_TELEGRAM_PARSER_PORT=${PORT}" \
