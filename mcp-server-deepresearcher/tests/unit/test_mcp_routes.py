...
async def test_mcp_deep_research_success(mock_mcp_context):
    ...
    request = DeepResearchRequest(research_topic="machine learning")
...
async def test_mcp_deep_research_missing_llm():
    ...
    ctx = MagicMock(spec=Context)
    ctx.request_context.lifespan_context = {"llm": None, "mcp_tools": [MagicMock()]}
...
async def test_mcp_deep_research_missing_tools():
    ...
    ctx = MagicMock(spec=Context)
    ctx.request_context.lifespan_context = {"llm": MagicMock(), "mcp_tools": None}
...
async def test_mcp_deep_research_empty_summary(mock_mcp_context):
    ...
    mock_result = {"running_summary": {}, "report": {}}
...
async def test_mcp_deep_research_langfuse_integration(mock_mcp_context, monkeypatch):
    ...
    monkeypatch.setattr(
        "mcp_server_deepresearcher.server.LangfuseConfig", lambda: mock_langfuse_config
    )

    with (
        patch("mcp_server_deepresearcher.server.DeepResearcher") as mock_agent_class,
