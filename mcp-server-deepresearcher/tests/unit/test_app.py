...
async def test_create_app_returns_fastapi_app(monkeypatch):
    ...
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.setup_spare_llm", mock_setup_spare_llm
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.initialize_llm", mock_initialize_llm
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.load_mcp_servers_config", mock_load_config
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.MultiServerMCPClient",
        lambda cfg: MagicMock(get_tools=AsyncMock(return_value=mock_tools)),
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.get_x402_settings",
        lambda: MagicMock(
            pricing_mode="off", pricing={}, validate_against_routes=lambda x: None
        ),
    )
...
async def test_app_lifespan_initializes_resources(monkeypatch):
    ...
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.MultiServerMCPClient", mock_client_factory
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.construct_tools_description_yaml",
        mock_construct_yaml,
    ...
    async with app_lifespan(app):
        ...
        )  # mock_llm.with_fallbacks returns mock_llm in this test
...
async def test_create_app_x402_middleware_configuration(monkeypatch):
    ...
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.get_x402_settings", lambda: mock_settings_on
    )
    ...
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.get_x402_settings", lambda: mock_settings_off
    )
