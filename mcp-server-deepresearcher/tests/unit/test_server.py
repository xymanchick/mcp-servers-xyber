...
async def test_deep_research_success(mock_context, mock_agent):
    ...
    with patch(
        "mcp_server_deepresearcher.deepresearcher.graph.DeepResearcher",
...
async def test_deep_research_agent_error(mock_context, mock_agent_with_error):
    ...
    with patch(
        "mcp_server_deepresearcher.server.DeepResearcher",
        return_value=mock_agent_with_error,
    ):
...
async def test_app_lifespan_initialization(monkeypatch):
    ...
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.setup_spare_llm", lambda: mock_spare_llm
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.load_mcp_servers_config",
        lambda **kwargs: {"mock": "config"},
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.MultiServerMCPClient", lambda cfg: mock_client
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.construct_tools_description_yaml",
        lambda tools: "tools:\n  - name: test",
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.parse_tools_description_from_yaml",
        lambda yaml_str: [{"name": "test", "description": "test"}],
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.initialize_llm",
...
async def test_app_lifespan_partial_tools_failure(monkeypatch):
    ...
    )  # Only one tool instead of expected multiple
...
async def test_concurrent_deep_research_requests(multiple_contexts, mock_agent_factory):
    with patch(
        "mcp_server_deepresearcher.server.DeepResearcher",
        side_effect=mock_agent_factory,
    ):
        deep_research_func = await get_deep_research_func()
        tasks = [
            deep_research_func(
                multiple_contexts[0], DeepResearchRequest(research_topic="topic1")
            ),
            deep_research_func(
                multiple_contexts[1], DeepResearchRequest(research_topic="topic2")
            ),
            deep_research_func(
                multiple_contexts[2], DeepResearchRequest(research_topic="topic3")
            ),
        ]
...
async def test_app_lifespan_cleanup_on_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "mcp_server_deepresearcher.server.setup_llm", lambda cfg: mock_llm
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.server.setup_spare_llm", lambda cfg: mock_spare_llm
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.load_mcp_servers_config",
        lambda **kwargs: {"server1": {"url": "http://test"}},
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.server.MultiServerMCPClient", mock_client_factory
    )
...
async def test_app_lifespan_with_partial_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "mcp_server_deepresearcher.app.load_mcp_servers_config", lambda **kwargs: {}
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.server.MultiServerMCPClient", lambda cfg: mock_client
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.server.construct_tools_description_yaml",
        lambda tools: "",
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.server.parse_tools_description_from_yaml",
        lambda yaml_str: [],
    )
    monkeypatch.setattr(
        "mcp_server_deepresearcher.server.initialize_llm",
        lambda llm_type, raise_on_error: None,
    )
...
async def test_json_serialization_edge_cases():
    ...
    mock_agent.graph.ainvoke = AsyncMock(
        return_value={
            "running_summary": {"result": None, "sources": None, "query_count": None}
        }
    )
