...
class ResearchGraph:
    ...
    def _build_graph(self) -> StateGraph:
        # Add nodes and edges
        builder = StateGraph(
            ResearchState, input_schema=ResearchState, output_schema=ResearchState
        )
        # Add nodes
    ...
    async def generate_query(self, state: ResearchState, config: RunnableConfig):
        ...
        formated_prompt = query_writer_instructions.format(
            ...
            tools_description=ToolDescription.format_list_for_prompt(
        ...
        try:
            ...
            logger.warning(
                f"JsonOutputParser failed, falling back to json.loads(): {parser_error}"
            )
        ...
        try:
            ...
            if not isinstance(tools_to_use, list):
                logger.warning(
                    f"Tools field is not a list, got {type(tools_to_use)}. Converting to list."
                )
    ...
    async def web_research(self, state: ResearchState, config: RunnableConfig):
        ...
        if state.tools_to_use:
            selected_tools = [
                tool for tool in self.tools if tool.name in state.tools_to_use
            ]
            logger.info(
                f"Selected {len(selected_tools)} tools from {len(state.tools_to_use)} requested: {[t.name for t in selected_tools]}"
            )
        else:
            # If no tools specified, use all available tools
            selected_tools = self.tools
            logger.info(
                f"No tools specified, using all {len(selected_tools)} available tools"
            )
        ...
        for i, result in enumerate(parallel_results):
            ...
            if isinstance(result, Exception):
                ...
                if "404" in error_msg or "Not Found" in error_msg:
                    ...
                    logger.warning(
                        f"  - Task '{task_name}' returned 404/Not Found: {error_detail}"
                    )
                    ...
                    logger.error(
                        f"  - Task '{task_name}' HTTP error {http_code}: {error_msg}"
                    )
                    ...
                    logger.error(
                        f"  - Task '{task_name}' connection error: {error_msg}"
                    )
                else:
                    error_detail = f"Error ({error_type}): {error_msg[:200]}"
                    logger.error(
                        f"  - Task '{task_name}' failed with {error_type}: {error_msg}"
                    )
            ...
            logger.info(
                f"  - Task '{task_name}' completed successfully. Adding raw content."
            )
            all_raw_content.append(
                "Search Results from "
                + task_name
            ...
            logger.info(
                f"  - Extracted {len(tool_sources)} sources from '{task_name}' results."
            )
        ...
        for content in all_raw_content:
            if (
                content.startswith("Note: The search tool")
                or "encountered an error" in content
            ):
        ...
        logger.info(
            f"Task execution summary: {successful_tasks}/{len(parallel_results)} tasks succeeded"
        )
        if failed_tasks:
            logger.warning(
                f"Failed tasks ({len(failed_tasks)}): {[t['task'] for t in failed_tasks]}"
            )
            for failed_task in failed_tasks:
                logger.warning(
                    f"  - {failed_task['task']}: {failed_task['error_detail']}"
                )
            ...
            for source_item in state.sources_gathered:
                if isinstance(source_item, dict):
                    ...
                    for line in lines:
                        if (
                            not line.strip()
                            or line.strip() == "No valid sources found."
                        ):
                            continue
                        match = re.match(
                            r"\d+\.\s*(?:\[([^\]]+)\]\s*)?([^(]+)\s*\(([^)]+)\)",
        ...
        existing_urls = {
            src.get("url", "N/A").rstrip("/")
            for src in unique_existing_sources
            if src.get("url") and src.get("url") != "N/A"
        }
        ...
        for src in unique_new_sources:
            ...
            if url and url != "N/A":
                if url.rstrip("/") not in existing_urls:
                    ...
                    existing_urls.add(
                        url.rstrip("/")
                    )  # Ensure no duplicates within the new list
            else:
                # For sources without URLs, add if the title is new among no-URL sources
                existing_titles = {
                    s.get("title", "").strip().lower()
                    for s in unique_existing_sources
                    if not s.get("url") or s.get("url") == "N/A"
                }
        ...
        logger.info(
            f"Sources this cycle: {len(all_structured_sources)} -> {len(unique_new_sources)} unique. "
        ...
        logger.info(
            f"All unique sources ({len(final_unique_sources)} total): {formatted_sources_str[:500]}..."
        )
    ...
    async def summarize_sources(self, state: ResearchState) -> ResearchState:
        ...
        logger.info(
            f"Found {len(web_research_results)} web research result(s) in state"
        )
        ...
        if total_chars > MAX_CHARS:
            logger.warning(
                f"Web research results exceed {MAX_CHARS:,} characters. Truncating..."
            )
            truncated_results = []
            current_chars = 0
                ...
                if current_chars + result_chars <= MAX_CHARS:
                    ...
                    if (
                        ...
                        truncated_results.append(
                            result_str[:remaining_chars] + "\n\n[TRUNCATED...]"
                        )
                    break
            web_research_results = truncated_results
            logger.info(
                f"Truncated to {sum(len(str(r)) for r in web_research_results):,} characters"
            )
    ...
    async def generate_report(self, state: ResearchState) -> dict:
        ...
        if state.sources_gathered:
            ...
            logger.info(
                f"Added {len(unique_sources)} unique sources to final report (from {len(all_sources)} total accumulated)"
            )
