...
def setup_llm(LLM_Config: LLM_Config):
    ...
    if MODEL_PROVIDER == "together":
        if not TOGETHER_API_KEY:
            logger.error(
                "TOGETHER_API_KEY environment variable is required for Together AI"
            )
            raise ValueError(
                "TOGETHER_API_KEY environment variable is required for Together AI"
            )
        ...
        if not MISTRAL_API_KEY:
            logger.error(
                "MISTRAL_API_KEY environment variable is required for Mistral AI"
            )
            raise ValueError(
                "MISTRAL_API_KEY environment variable is required for Mistral AI"
            )
...
def setup_spare_llm(LLM_Config: LLM_Config):
    ...
    logger.info(
        f"Using model provider: {MODEL_PROVIDER_SPARE}, model: {MODEL_NAME_SPARE}"
    )
...
def create_mcp_tasks(mcp_tools, search_query):
    ...
    for tool in mcp_tools:
        ...
        # This is where you match and activate the Apify tool
            ...
            actor_input = {"searchTerms": [search_query], "limit": 15}
