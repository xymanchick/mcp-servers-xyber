...
class ResearchState:
    ...
    simplified_search_query: str = field(
        default=None
    )  # Simple query for Twitter/Apify tools
    ...
    tools_description: list[ToolDescription] = field(
        default_factory=list
    )  # Available tools description
    tools_to_use: list[str] = field(default_factory=list)  # Tools to use
    stop_research: bool = field(
        default=False
    )  # Flag to stop research if summary is good enough
