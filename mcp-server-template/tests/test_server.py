...
class TestGetWeatherTool:
    ...
    def mock_context(self):
        ...
        context.request_context.lifespan_context = {"weather_client": Mock()}
        return context
    ...
    async def test_get_weather_success(self, mock_context):
        ...
        weather_client.get_weather = AsyncMock(
            return_value=WeatherData(state="sunny", temperature="25.5C", humidity="60%")
        )
        ...
        # Verify result
        assert result == {"state": "sunny", "temperature": "25.5C", "humidity": "60%"}
    ...
    async def test_get_weather_with_coordinates(self, mock_context):
        ...
        weather_client.get_weather = AsyncMock(
            return_value=WeatherData(
                state="cloudy", temperature="18.3C", humidity="75%"
            )
        )

        # Call the tool with coordinates
        result = await _get_weather_impl(
            mock_context, latitude="40.7128", longitude="-74.0060"
        )

        # Verify result
        assert result == {"state": "cloudy", "temperature": "18.3C", "humidity": "75%"}
    ...
    async def test_get_weather_api_error(self, mock_context):
        ...
        weather_client = mock_context.request_context.lifespan_context["weather_client"]
        weather_client.get_weather = AsyncMock(
            side_effect=WeatherApiError("API key invalid")
        )
    ...
    async def test_get_weather_client_error(self, mock_context):
        ...
        weather_client = mock_context.request_context.lifespan_context["weather_client"]
        weather_client.get_weather = AsyncMock(
            side_effect=WeatherClientError("Failed to parse data")
        )

        # Call the tool and expect error
        with pytest.raises(
            ToolError, match="Weather client error: Failed to parse data"
        ):
            await _get_weather_impl(mock_context, "51.5074", "-0.1278")
    ...
    async def test_get_weather_unexpected_error(self, mock_context):
        ...
        # Call the tool and expect error
        with pytest.raises(
            ToolError, match="An unexpected error occurred processing weather request"
        ):
            await _get_weather_impl(mock_context, "51.5074", "-0.1278")
