...
class TestRetryApiCallDecorator:
    ...
    def test_retry_on_weather_api_error_then_success(self, weather_client, caplog):
        ...
        responses = [
            Mock(
                status_code=429,
                json=lambda: {"cod": 429, "message": "Too many requests"},
            ),
            Mock(
                status_code=503,
                json=lambda: {"cod": 503, "message": "Service unavailable"},
            ),
    ...
    def test_max_retries_exceeded(self, weather_client, caplog):
        ...
        with patch(
            "requests.get", side_effect=ConnectionError("Connection failed")
        ) as mock_get:
            with caplog.at_level(logging.WARNING):
