...
class GameState(BaseModel):
    ...
    remaining_attempts: int = Field(..., ge=0, description="Attempts left before losing")
    correct_letters: list[str] = Field(default_factory=list, description="Correctly guessed letters")
    incorrect_letters: list[str] = Field(default_factory=list, description="Incorrectly guessed letters")
