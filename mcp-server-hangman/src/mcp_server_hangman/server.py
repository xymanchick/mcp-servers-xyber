...
async def start_game(
    ctx: Context,
    secret_word: Annotated[str, Field(description="Secret word to guess (letters only)")],
    max_attempts: Annotated[int, Field(ge=1, description="Maximum wrong attempts allowed")] = 6,
) -> GameState:
    """Start or reset a Hangman game and return a new player_id for subsequent guesses."""
    # Generate a unique player/session id
    player_id = uuid.uuid4().hex
    try:
        service: HangmanService = ctx.request_context.lifespan_context["hangman_service"]
        return service.start_game(player_id=player_id, secret_word=secret_word, max_attempts=max_attempts)
...
async def guess_letter(
    ctx: Context,
    player_id: Annotated[str, Field(description="Player/session id returned by start_game")],
