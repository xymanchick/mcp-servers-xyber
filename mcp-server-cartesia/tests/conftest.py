...
def mock_cartesia_tool():
    ...
    tool.arun.return_value = "Successfully generated speech and saved to: /app/audio_outputs/test_output.wav"
    return tool
...
def mock_react_agent():
    ...
    mock_response = {
        "messages": [
            Mock(content="Agent successfully generated speech")
        ]
    }
...
def mock_context_with_error_service(mock_cartesia_service_with_error):
    ctx = AsyncMock(spec=Context)
    ctx.request_context.lifespan_context = {"cartesia_service": mock_cartesia_service_with_error}
    return ctx
